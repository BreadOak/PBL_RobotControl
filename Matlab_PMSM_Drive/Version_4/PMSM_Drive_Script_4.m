...
Control_mode = 4;          % [1:Current(MPC), 2:Velocity, 3: Position]
...
Gam_wme = [gam_wme, -gam_wme;
           -gam_wme, gam_wme];

Delta_t = Tsw;

% Continuous Model
Ac = [-Rs/Lds,                     0,  Lqs/Lds,          0;
            0,               -Rs/Lqs,        0, -LAMpm/Lqs;
            0,                     0,        0,          0;
            0, 1.5*PolePair*LAMpm/Jm,        0,     -Bm/Jm];
...
% Discrete Model
Ad = [ 1-Delta_t*Rs/Lds,                             0, Delta_t*Lqs/Lds,                  0;
                      0,              1-Delta_t*Rs/Lqs,               0, -Delta_t*LAMpm/Lqs;
                      0,                             0,               1,                  0;
                      0, Delta_t*1.5*PolePair*LAMpm/Jm,               0,    1-Delta_t*Bm/Jm];
...
Aa = [        Ad, zeros(4,1),         Bd;
      ...
      zeros(2,4), zeros(2,1),   eye(2,2)];

Ba = [zeros(5,2);
        eye(2,2)];

Q = [    Gam_i, zeros(2,1), zeros(2,4);
    zeros(1,2), zeros(1,1), zeros(1,4);
    zeros(2,3),    Gam_wme, zeros(2,2);
    zeros(2,2), zeros(2,1), zeros(2,4)];
