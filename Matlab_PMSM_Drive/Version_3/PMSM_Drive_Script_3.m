...
Control_mode = 2;          % [1:Current(MPC), 2:Velocity, 3: Position]
...
%         Vdse_Integ_Ref(i) = Vdse_Integ_Ref(i-1) + KidT_cc * (Err_Idse - (Kpd_Anti_cc*Vdse_Ref_Anti(i))); % I part
%         Vdse_Ref_Fb = Vdse_Integ_Ref(i) + Kpd_cc * Err_Idse;                                             % PI part
%         Vdse_Ref_Ff = Wr(i)*Lds*Iqse(i);                                                                 % back emf
%         Vdse_Ref(i) = Vdse_Ref_Fb + Vdse_Ref_Ff;
% 
%         Err_Iqse = Iqse_Ref(i) - Iqse(i);
%         Vqse_Integ_Ref(i) = Vqse_Integ_Ref(i-1) + KiqT_cc * (Err_Iqse - (Kpq_Anti_cc*Vqse_Ref_Anti(i))); % I part
%         Vqse_Ref_Fb = Vqse_Integ_Ref(i) + Kpq_cc * Err_Iqse;                                             % PI part
%         Vqse_Ref_Ff = Wr(i)*Lds*Idse(i) + Wr(i) * LAMpm;                                                 % back emf
%         Vqse_Ref(i) = Vqse_Ref_Fb + Vqse_Ref_Ff;      

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Model Precictice Control
        r = [Idse_Ref(i); Iqse_Ref(i)];

        Am = [-Rs/Lds, Wr(i)*(Lqs/Lds); 
             -Wr(i)*(Lds/Lqs), -Rs/Lqs];
...
Error_MPC_c = Iqse_Ref*1.5*PolePair*LAMpm - M_Te;
Error_MPC_v = Wrm_Ref*Rm2rpm - M_Wrm*Rm2rpm;
rms(Error_MPC_v)
